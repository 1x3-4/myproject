...
<!-- 7. 현재 페이지블록번호 : pageBlock -->
<c:choose>
	<c:when test="${empty param.pageBlock}">
...
<!-- 8. 마지막/총 페이지번호 : total_pageNum -->
<fmt:parseNumber var="tPageNum"
	value="${Math.ceil(totalRows/rows_page)}" integerOnly="true" />
<c:set var="total_pageNum" value="${tPageNum}" />

<!-- 9. 마지막 페이지블록번호 : last_pageBlock -->
<fmt:parseNumber var="lPageBlock"
	value="${Math.ceil(total_pageNum/pages_pageBlock)}" integerOnly="true" />
	...
	<table border="2">
	  ...
	  -->
			...
			<c:otherwise>
				<c:forEach var="i" begin="${startNum}" end="${endNum}">
					<c:if test="${not empty boardList[i-1]}">
						...
						<tr>
							<td>${i}</td>
							<td><a href="view.jsp?b_idx=${boardList[i-1].b_idx}">
									${boardList[i-1].title} </a></td>
							<td>${boardList[i-1].writer}</td>
							<td>${boardList[i-1].post_date}</td>
							<td>${boardList[i-1].read_cnt}</td>
							<td>
								<!-- 첨부파일이 있는 경우에는 정해진 이미지를 출력시키고 없는 경우에는 공란으로 처리함 --> <c:if
									test="${not empty boardList[i-1].originfile_name}">
									<a
										href="download.jsp?originfile_name=${boardList[i-1].originfile_name}&savefile_name=${boardList[i-1].savefile_name}">
										<img src="../resources/css/img/download.png" alt="첨부파일 이미지"
		...
		<c:if test="${not empty member}">
			<tr>
				<td colspan="6"><a href="write.jsp"><input type="button"
						value="글등록"></a></td>
			</tr>
		</c:if>
		...
		<c:if test="${not empty boardList}">
			<tr>
				<td colspan="6" id="td_paging"><%@ include file="paging.jsp"%>
				</td>
			</tr>
		</c:if>
