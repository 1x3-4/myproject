...
public class BoardDAO extends DBConnection {
	...
	public ArrayList<BoardDTO> getBoards() {
		...
		String sql = "select * from tb_board" + " where del_or_not = 1" + " order by b_idx desc";

		// order by b_idx desc : 게시판의 글 목록은 최근 글이 위에 위치하므로 게시글 번호를 기준으로 내림차순 정렬하여 조회한다
		try {
			pstmt = conn.prepareStatement(sql);
			...
			System.out.println("게시판 목록 조회 중 예외발생");
			e.printStackTrace();
		}
	...
	public void updateRead_cnt(int b_idx) {
		...
		try {
			...
			pstmt.executeUpdate(); // 조회된 결과를 ResultSet 객체에 담음
		} catch (Exception e) {
			System.out.println("조회수 업데이트 중 예외발생");
			e.printStackTrace();
		}
	...
	public BoardDTO getBoard(int b_idx) {
		...
		try {
			...
			System.out.println("게시판 조회 중 예외발생");
			e.printStackTrace();
		}
	...
	public int delete(int b_idx) {
		int result = 0; // 게시글 삭제 처리 실패시 결과값
		try

		{
			String sql = "update tb_board set del_or_not = 0" + " where b_idx=?";
			...
			System.out.println("게시글 삭제 처리중 예외발생");
			e.printStackTrace();
		}
		return result;
...
//				검색영역에 따라 구분을 나눔 : switch
				...
				sql = "select * from tb_board " + " where del_or_not = 1 " + " and " + searchField
						+ " like '%'||?||'%' " + " order by b_idx desc";

				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, sDto.getSearchWord());

			} else { // 검색어가 입력되지 않은 경우
				sql = "select * from tb_board" + " where del_or_not = 1" + " order by b_idx desc";
				pstmt = conn.prepareStatement(sql);
			}
